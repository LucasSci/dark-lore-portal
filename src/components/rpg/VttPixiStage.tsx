...
export default memo(function VttPixiStage({
  ...
  useEffect(() => {
    ...
    // ⚡ Bolt Optimization: Batch fog rendering to prevent massive GC overhead
    ...
    // ⚡ Bolt Optimization: Batch wall rendering to prevent massive GC overhead
    const batchedWalls = new Graphics();
    const batchedDots = new Graphics();
    let hasWalls = false;
    ...
    for (const wall of page.wallSegments) {
      ...
      batchedDots.circle(ax, ay, 3);
      batchedDots.circle(bx, by, 3);
    }

    if (hasWalls) {
      batchedWalls.stroke({ color: 0xe85d4a, alpha: boardMode === "wall" ? 0.85 : 0.35, width: boardMode === "wall" ? 3 : 2 });
      batchedDots.fill({ color: 0xe85d4a, alpha: 0.8 });
      wallLayer.addChild(batchedWalls, batchedDots);
    }
