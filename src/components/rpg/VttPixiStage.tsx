...
export default memo(function VttPixiStage({
  ...
  useEffect(() => {
    ...
    // ⚡ Bolt: Batch grid cells and fog into single Graphics objects to reduce PIXI display objects
    ...
    batchedInteraction.on("pointertap", (event) => {
      const local = batchedInteraction.toLocal(event.global);
    ...
    for (const cell of page.cells) {
      const px = cell.x * page.gridSize;
      const py = cell.y * page.gridSize;

      batchedInteraction.rect(px, py, page.gridSize, page.gridSize);

      if (!page.fog[cell.id]) {
        batchedFog.rect(px, py, page.gridSize, page.gridSize);
      }
    }
