...
export default function GameMasterPanel() {
  ...
  const nextChapter = useMemo(() => {
    // ⚡ Bolt: Optimize nextChapter bounds calculation
    // What: Replace Math.max(...array.map()) with a single-pass loop.
    // Why: Avoids creating intermediate arrays, reducing GC pressure and preventing maximum call stack size exceeded errors on large campaign sets.
    // Impact: Lower memory overhead and safer execution for large arrays.
    let max = 1;
