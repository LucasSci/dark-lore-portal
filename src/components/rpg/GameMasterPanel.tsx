...
export default function GameMasterPanel() {
  ...
  const nextChapter = useMemo(() => {
    // ⚡ Bolt: Replace Math.max(...arr.map()) with a single-pass for loop
    // What: Find maximum chapter number iteratively instead of using spread and map.
    // Why: Spreading mapped arrays risks "Maximum call stack size exceeded" errors on large collections and allocates unnecessary intermediate arrays.
    // Impact: Eliminates O(N) memory allocations, reduces garbage collection pressure, and makes calculation inherently safe for massive datasets.
    let max = 1;
