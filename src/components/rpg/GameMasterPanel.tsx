...
export default function GameMasterPanel() {
  ...
  const nextChapter = useMemo(() => {
    // ⚡ Bolt: Use single-pass for-loop instead of Math.max(...arr.map())
    // This prevents "Maximum call stack size exceeded" on large publication
    // sets and eliminates intermediate array allocation GC overhead.
    let max = 1;
