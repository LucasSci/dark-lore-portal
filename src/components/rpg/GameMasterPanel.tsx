...
export default function GameMasterPanel() {
  ...
  const nextChapter = useMemo(() => {
    // ⚡ Bolt: Replace spread syntax and map with a single-pass for loop.
    // What: Replace `Math.max(1, ...publications.map(p => p.chapterNumber))` with a manual for loop.
    // Why: Prevents "Maximum call stack size exceeded" errors on large collections and avoids GC overhead from intermediate array mapping.
    // Impact: Improves memory efficiency and protects against call stack limits.
    let max = 1;
