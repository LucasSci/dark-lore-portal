...
export default function GameMasterPanel() {
  ...
  const nextChapter = useMemo(() => {
    // ⚡ Bolt: Single-pass for loop to calculate max chapter
    // What: Replace Math.max(...arr.map()) with a single-pass for loop.
    // Why: Protects against "Maximum call stack size exceeded" exceptions on large collections and eliminates array mapping garbage collection overhead.
    // Impact: Reduces GC allocations and guarantees stability regardless of the number of publications.
    let max = 1;
