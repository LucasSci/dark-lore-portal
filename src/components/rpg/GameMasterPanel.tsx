...
export default function GameMasterPanel() {
  ...
  const nextChapter = useMemo(() => {
    // ⚡ Bolt: Calculate max chapter using single-pass loop
    // What: Replaced Math.max(...publications.map()) with a for loop
    // Why: Prevents call stack limit errors on large datasets and eliminates intermediate array allocation GC overhead
    // Impact: O(1) memory instead of O(N), safer and faster for large campaigns
    let max = 1;
