...
export default function GameMasterPanel() {
  ...
  const nextChapter = useMemo(() => {
    // ⚡ Bolt: Replace map+spread Math.max with single-pass loop
    // What: Replaced spread operator and array map with a simple for loop to find max chapter.
    // Why: Prevents maximum call stack size exceeded errors on large lists and eliminates intermediate array allocation GC overhead.
    // Impact: Improved performance and memory safety for large campaign publication lists.
    let maxChapter = 1;
