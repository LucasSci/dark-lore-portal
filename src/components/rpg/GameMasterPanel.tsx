...
export default function GameMasterPanel() {
  ...
  const nextChapter = useMemo(() => {
    // ⚡ Bolt: Single-pass loop to find max chapter without spread syntax
    // What: Replace Math.max(...arr.map()) with a single-pass loop.
    // Why: Prevents 'Maximum call stack size exceeded' on large arrays and eliminates GC overhead from intermediate mapping.
    // Impact: Improves memory stability and execution speed for large collections.
    let max = 1;
