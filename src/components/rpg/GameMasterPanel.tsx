...
export default function GameMasterPanel() {
  ...
  const nextChapter = useMemo(() => {
    // ⚡ Bolt: Replace Math.max(...array.map()) with a single-pass loop
    // What: Calculates the maximum chapter number using a for loop instead of array spread.
    // Why: Prevents "Maximum call stack size exceeded" errors for large arrays and eliminates GC overhead from intermediate .map() arrays.
    // Impact: O(n) execution without allocating intermediate arrays or hitting argument limits.
    let max = 1;
