...
export default function GameMasterPanel() {
  ...
  const nextChapter = useMemo(() => {
    // ⚡ Bolt: Replace array map and spread with single-pass loop
    // What: Replaced `Math.max(1, ...publications.map())` with a `for` loop.
    // Why: To avoid allocating an intermediate array and to prevent potential "Maximum call stack size exceeded" errors if `publications` grows excessively large.
    // Impact: Reduces garbage collection overhead and improves scalability for large arrays.
    let maxChapter = 1;
