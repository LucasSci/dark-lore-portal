...
export default function GameMasterPanel() {
  ...
  const nextChapter = useMemo(() => {
    // ⚡ Bolt: Replace array spreading with a single-pass for loop
    // What: Computes the maximum chapter number using a for loop instead of Math.max(...array.map()).
    // Why: Avoids Maximum call stack size exceeded errors on large datasets and eliminates GC pressure from intermediate arrays.
    // Impact: Improves memory efficiency and ensures safe scaling for user-generated datasets.
    let max = 1;
