...
export default function GameMasterPanel() {
  ...
  const nextChapter = useMemo(
    () => {
      // ⚡ Bolt: Optimize max calculation
      // What: Replace Math.max(...array.map()) with a single-pass loop
      // Why: Eliminates mapping allocation overhead and protects against call stack size exceeded errors
      // Impact: Reduces GC pressure and improves resilience for large collections
      let maxChapter = 1;
