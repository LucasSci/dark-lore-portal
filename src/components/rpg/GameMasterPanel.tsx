...
export default function GameMasterPanel() {
  ...
  const nextChapter = useMemo(() => {
    // ⚡ Bolt: Use a single-pass loop instead of `.map()` and spread operator to avoid stack overflow and GC pressure on large datasets
    // What: Replaced `Math.max(1, ...publications.map(p => p.chapterNumber))` with a `for` loop
    // Why: The spread operator can exceed call stack limits for large publications arrays, and `.map()` creates unnecessary intermediate arrays
    // Impact: Reduces GC pressure and prevents potential call stack errors for campaigns with many publications
    let maxChapter = 1;
