...
export default function GameMasterPanel() {
  ...
  const nextChapter = useMemo(() => {
    // ⚡ Bolt: Use single-pass loop instead of spread operator and map
    // What: Replace Math.max(...map()) with a standard for loop
    // Why: Prevents "Maximum call stack size exceeded" errors on large lists and avoids allocating an intermediate array
    // Impact: O(n) iteration with O(1) memory footprint instead of O(n) memory, ensuring stability at scale
    let maxChapter = 1;
