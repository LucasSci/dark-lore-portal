...
export default function GameMasterPanel() {
  ...
  const nextChapter = useMemo(() => {
    // ⚡ Bolt: Replace Math.max(...map) with a single-pass for loop
    // What: Find maximum chapter number manually without spread syntax or mapping
    // Why: Prevents 'Maximum call stack size exceeded' on large datasets and avoids intermediate array allocation
    // Impact: Reduces GC overhead and execution time when computing next chapter number
    let maxChapter = 1;
