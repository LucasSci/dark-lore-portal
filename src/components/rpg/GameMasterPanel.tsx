...
export default function GameMasterPanel() {
  ...
  const nextChapter = useMemo(() => {
    // ⚡ Bolt: single-pass for loop
    // What: Replace Math.max(...array.map()) with a single loop
    // Why: Prevent call stack limits on large arrays and avoid GC allocation from .map()
    // Impact: Eliminates O(N) intermediate array creation and O(N) call stack expansion.
    let max = 1;
