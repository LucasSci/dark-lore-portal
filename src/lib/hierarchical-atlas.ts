...
export function getPolygonBounds(points: AtlasCoordinate[]): AtlasBounds {
  // ⚡ Bolt: Avoid intermediate array allocations and spread operator overhead
  // Use a single-pass loop to find min/max values directly.
  if (points.length === 0) {
    return { southWest: { x: 0, y: 0 }, northEast: { x: 0, y: 0 } };
  }
