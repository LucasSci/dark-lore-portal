...
export const RACES = [
  { value: 'humano', label: 'Humano', bonus: { any: 1 }, desc: 'Versátil e adaptável' },
  { value: 'elfo', label: 'Elfo', bonus: { destreza: 2 }, desc: 'Gracioso e perceptivo' },
  { value: 'anao', label: 'Anão', bonus: { constituicao: 2 }, desc: 'Resistente e robusto' },
  { value: 'orc', label: 'Orc', bonus: { forca: 2 }, desc: 'Forte e feroz' },
  { value: 'tiefling', label: 'Tiefling', bonus: { carisma: 2 }, desc: 'Infernal e carismático' },
  { value: 'draconato', label: 'Draconato', bonus: { forca: 2 }, desc: 'Descendente de dragões' },
...
export const CLASSES = [
  ...
  { value: 'clerigo', label: 'Clérigo', hitDie: 8, primaryAttr: 'sabedoria', mpBase: 6 },
  ...
  { value: 'barbaro', label: 'Bárbaro', hitDie: 12, primaryAttr: 'forca', mpBase: 0 },
...
export const ATTRIBUTES = [
  { key: 'forca', label: 'Força', abbr: 'FOR', icon: '⚔️' },
  { key: 'destreza', label: 'Destreza', abbr: 'DES', icon: '🏹' },
  { key: 'constituicao', label: 'Constituição', abbr: 'CON', icon: '🛡️' },
  { key: 'inteligencia', label: 'Inteligência', abbr: 'INT', icon: '📖' },
  { key: 'sabedoria', label: 'Sabedoria', abbr: 'SAB', icon: '🔮' },
  { key: 'carisma', label: 'Carisma', abbr: 'CAR', icon: '👑' },
] as const;
...
export function calculateHP(classValue: string, conMod: number, level: number): number {
  ...
  if (!cls) return 10;
  return cls.hitDie + conMod + (level - 1) * (Math.ceil(cls.hitDie / 2) + 1 + conMod);
}

export function calculateMP(classValue: string, intOrWisMod: number): number {
  const cls = CLASSES.find(c => c.value === classValue);
  if (!cls) return 0;
  return Math.max(0, cls.mpBase + intOrWisMod);
}
