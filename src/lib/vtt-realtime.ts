...
export async function loadSceneSnapshot(sessionId: string) {
  ...
  try {
    ...
    // ⚡ Bolt: Avoid array mapping and spreading for bounds calculation
    let sceneRevision = Math.max(1, Number(latestInitiative?.revision ?? 1));
    for (let i = 0; i < pages.length; i++) sceneRevision = Math.max(sceneRevision, Number(safePageRows[i]?.revision ?? 1));
    for (let i = 0; i < objects.length; i++) sceneRevision = Math.max(sceneRevision, objects[i].revision);
