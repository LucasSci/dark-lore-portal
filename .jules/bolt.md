...
## 2024-05-18 - Single-pass loop for RegExp building
**Learning:** When building a dynamic RegExp from a large array of objects (like mentions), using `array.map().join()` creates unnecessary intermediate arrays and forces multiple iterations, increasing GC pressure.
**Action:** Use a standard `for` loop to concatenate the pattern string and build associated data structures (like Maps) in a single pass to eliminate intermediate allocations.
