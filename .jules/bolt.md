...
## 2024-05-18 - Avoid chained array operations in hot text processing
**Learning:** In text processing functions, chaining `.map().join()` causes unnecessary intermediate array allocations, significantly increasing Garbage Collection (GC) overhead.
**Action:** Always replace `.map().join()` with a single-pass `for` loop and string concatenation to build strings, and use the same loop to build associated data structures directly.
