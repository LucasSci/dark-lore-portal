...
## 2024-05-18 - Avoid Math.max with Spread Operator on User Data
**Learning:** Using Math.max(...array.map()) on user-generated datasets like publications can exceed the JS engine's maximum call stack size limit as the data grows, causing runtime crashes and unnecessary GC pressure.
**Action:** Use single-pass for loops to update primitive tracking variables instead of mapping arrays and spreading them into variadic functions.
