...
## 2026-08-31 - Avoid Spread with Math.max on large arrays
**Learning:** Using `Math.max(...array.map())` on potentially large arrays (like campaign publications) creates intermediate arrays causing GC pressure and can exceed the JavaScript engine's maximum call stack size limit.
**Action:** Use a single-pass `for` loop to find maximum values over large datasets to use O(1) memory and avoid call stack limits.
