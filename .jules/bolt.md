...
## 2024-05-18 - Batching large sets of PIXI Graphics
**Learning:** Creating a new `Graphics` object for every grid cell (and its fog) on large VTT boards leads to immense overhead and sluggish rendering because thousands of individual `Graphics` entities are instantiated in the hot loop inside `VttPixiStage.tsx`. When batched into a single object, pointer interactivity (`pointertap`) is lost for individual cells unless explicitly handled via math.
**Action:** Always batch repeated shapes (like grid tiles or fog overlays) into a single `Graphics` object using multiple `rect` calls. Maintain individual interactivity by setting `eventMode="static"` on the batched object and implementing a custom hit-test in the `pointertap` handler by mapping the global pointer coordinates back to the logical grid using `toLocal(event.global)`.
