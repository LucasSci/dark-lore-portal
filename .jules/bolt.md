...
## 2024-05-18 - Batch PIXI Graphics for VTT Grid and Fog
**Learning:** Instantiating and adding thousands of `Graphics` objects to a scene graph per render loop (like grid cells or fog cells) introduces massive Garbage Collection (GC) and renderer overhead.
**Action:** Always batch repeated shapes into a single `Graphics` object via path operations (e.g. `rect()`, `moveTo()`) inside the loop, and apply styles/add to the stage just once outside the loop.
