...
## 2024-05-18 - Avoid Math.max(...arr.map()) for large collections
**Learning:** Using `Math.max(...arr.map())` creates intermediate arrays and uses the spread syntax which can cause "Maximum call stack size exceeded" on large datasets. This is also an O(n) operation with unnecessary garbage collection.
**Action:** Replace `Math.max(...arr.map())` with a single-pass `for` loop to update primitive max variables directly when finding the maximum value in large collections (e.g. `publications`).
