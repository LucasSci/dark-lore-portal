...
## 2024-05-18 - Math.min/Math.max inlining in hot loops
**Learning:** While replacing `Math.min` and `Math.max` with inline conditionals was historically a performance trick in high-frequency execution paths, modern JS engines (like V8) automatically inline them when called with two arguments.
**Action:** Avoid this micro-optimization as it provides virtually zero measurable performance improvement and can reduce readability. Rely on modern JS engine optimizations unless profiling proves otherwise.
