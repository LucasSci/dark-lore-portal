...
## 2024-05-24 - Batch PixiJS Shapes to Reduce GC
**Learning:** Creating thousands of individual `Graphics` objects for repeated grid cells or fog cells causes massive Garbage Collection (GC) overhead and performance degradation.
**Action:** Batch identical shapes by drawing them into a single `Graphics` object inside a loop (using `rect()`, `moveTo()`, `lineTo()`) and styling (`fill()`, `stroke()`) outside the loop. Use hit-testing on the parent container if individual events are needed.
