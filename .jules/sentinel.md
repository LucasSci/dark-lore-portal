...
## 2025-02-23 - Avoid Date.now() for unique identifiers
**Vulnerability:** Several components used Date.now() for identifier generation.
**Learning:** Date.now() is highly predictable and can generate duplicate IDs.
**Prevention:** Always use generateSecureId() or generateSecureShortId() for ID generation.
