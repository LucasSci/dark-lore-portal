...
## 2024-06-26 - Avoid Date.now() for backend asset paths
**Vulnerability:** File upload paths used `Date.now()` to generate file names, creating predictable identifiers for uploaded assets.
**Learning:** Backend identifiers or file paths should not rely on predictable data like `Date.now()`, as it allows path guessing and potential ID collisions during file uploads.
**Prevention:** Always use cryptographically secure utilities like `generateSecureShortId()` for asset paths.
