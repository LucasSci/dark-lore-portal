...
## 2026-06-13 - Stop committing secrets to .env files
**Vulnerability:** A tracked `.env` file was found in the repository containing hardcoded secrets (API keys) like `VITE_GEMINI_API_KEY` and Supabase credentials.
**Learning:** Including `.env` in the repository directly violates the security best practice of keeping secrets isolated from version control, making lateral movement or credential abuse easy for any user with repository access. Even if `.gitignore` has been updated, existing tracked files must be removed using `git rm --cached`.
**Prevention:** Ensure `.env` and `.env.*` (excluding `.env.example`) are explicitly defined in `.gitignore` from project inception and untrack any existing secrets immediately.
