...
## 2024-06-03 - Use cryptographically secure random IDs instead of Date.now()
**Vulnerability:** Predictable IDs were generated using `Date.now()` in asset upload paths.
**Learning:** Relying on predictable timestamps for unique path generation introduces the risk of collisions or predictability in the IDs.
**Prevention:** Always use secure generators like `generateSecureShortId()` from `src/lib/utils.ts`.
